...
and export as minimally impactful on phone energy, bandwidth and processing
...
some datastore that it has access to. We will seek to do minimal realtime
processing of the data. The first priority is for the server to quickly
...
because any heavy processing by any one request can tie up Node's event loop.
...
a service which runs continually in the background of the mobile phone.
Android has a class called \texttt{BroadcastReciever} which can messages
from multiple processes at the same time. An instrumented process merely needs
to know the right \texttt{Intent} to broadcast on, and arbitrary data can be
sent to the service. 
Code inserted into the App under test will communicate with the
...
next appropriate time to export them to the server.
...
app. Possible insertion points could be program structures such as the first
